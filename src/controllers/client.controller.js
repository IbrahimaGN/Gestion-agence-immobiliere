...
const createClient = asyncHandler(async (req, res) => {
  ...
  const { prenom, nom, email, telephone, agenceId, agence } = req.body;

  const idAgence = agenceId || agence;

  if (!idAgence) {
    throw new HttpError(400, "Le champ agenceId est obligatoire");
  }
  ...
  const agenceExistante = await agenceService.trouverAgenceParId(idAgence);

  if (!agenceExistante) {
    throw new HttpError(400, `L'agence avec l'ID ${idAgence} n'existe pas`);
  }
